defmodule Mix.Tasks.Mcp.Stdio do
  ...
  def run(args) do
    ...
    base_url = Keyword.get(opts, :url, "http://localhost:4001")
  ...
  defp run_direct_mode(log) do
    ...
    # Create a dedicated agent for this session to handle CodeGraph communication
    {:ok, agent} = Agent.start_link(fn -> %{session_id: session_id} end)
    log.("Started agent for session: #{inspect(agent)}")
    
    # Ensure the MCP server knows about this session
    log.("Initializing MCP session for #{session_id}")
    
    # Start handling STDIO
    handle_stdio(fn message ->
      try do
        # Use MCP.Server.handle_message/2 directly
        log.("Processing message: #{inspect(message)}")
        response = MCP.Server.handle_message(session_id, message)
        log.("Got response: #{inspect(response)}")
        
        case response do
          {:ok, nil} -> nil # For notifications that don't require a response
          {:ok, result} -> result
          {:error, {code, msg, data}} ->
            %{
              "jsonrpc" => "2.0",
