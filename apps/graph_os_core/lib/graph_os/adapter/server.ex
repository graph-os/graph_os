defmodule GraphOS.Adapter.Server do
  ...
  def init({adapter_module, opts}) do
    ...
    case adapter_module.init(opts) do
      ...
      {:error, reason} ->
        Logger.error(
          "Failed to initialize adapter #{inspect(adapter_module)}: #{inspect(reason)}"
        )
  ...
  def handle_call({:jsonrpc_request, request, context}, _from, %State{} = state) do
    ...
    if function_exported?(state.adapter_module, :handle_jsonrpc_request, 3) do
      case state.adapter_module.handle_jsonrpc_request(request, context, state.adapter_state) do
        {:reply, result, _updated_context, updated_adapter_state} ->
  ...
  def handle_call({:operation, operation, context}, _from, %State{} = state) do
    ...
    if Context.halted?(context_with_plugs) do
      ...
      error =
        Context.error(context_with_plugs) ||
          {:unknown_error, "Operation halted with no error specified"}

      {:reply, {:error, error}, state}
    else
      # Execute the operation in the adapter
      case state.adapter_module.handle_operation(
             operation,
             context_with_plugs,
             state.adapter_state
           ) do
