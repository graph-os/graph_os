defmodule GraphOS.Core.GitIntegration do
  @moduledoc """
  ...
  def list_branches(repo_path) do
    case System.cmd("git", ["--no-pager", "branch", "--list", "--format=%(refname:short)"], cd: repo_path) do
      {output, 0} ->
        branches = output
          |> String.split("\n", trim: true)
  ...
  def get_commits(repo_path, branch \\ nil, limit \\ 10) do
    ...
    case System.cmd(
      "git", 
      ["--no-pager", "log", branch_arg, "-n", "#{limit}", "--pretty=format:%H|%an|%ae|%at|%s"], 
      cd: repo_path
    ) do
      {output, 0} ->
        commits = output
  ...
  def blame(repo_path, file_path, options \\ []) do
    ...
    # Add options
    args = if Keyword.get(options, :line_range) do
  ...
  defp get_remote_url(repo_path) do
    case System.cmd("git", ["config", "--get", "remote.origin.url"], cd: repo_path) do
      {output, 0} -> {:ok, String.trim(output)}
      {_, _} -> {:ok, nil}  # No remote or no origin, not an error
    end
  end
  ...
  defp parse_blame_porcelain(output) do
    ...
    {_, result} = Enum.reduce(lines, {nil, []}, fn line, {current_chunk, acc} ->
      cond do
        # New commit line starts with the commit hash
        String.match?(line, ~r/^[0-9a-f]{40}/) ->
          [hash | rest] = String.split(line, " ", trim: true)
