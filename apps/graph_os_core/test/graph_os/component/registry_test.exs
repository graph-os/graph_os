defmodule GraphOS.Component.RegistryTest do
  ...
  # Start the registry once for all tests in this module
  setup_all do
    {:ok, _pid} = start_supervised(Registry)
    :ok
  end

  # Optional: Clear registry between tests if needed, but starting once might be sufficient
  # setup do
  #   :ok
  # end
  ...
  describe "lookup functions" do
    # Register components needed for lookup tests
    setup do
      :ok = Registry.register_many([TestComponent, AnotherComponent])
      # Return :ok to signal setup completion
      :ok
    end
  ...
  describe "list functions" do
    # Register components needed for list tests
    setup do
      :ok = Registry.register_many([TestComponent, AnotherComponent])
      # Return :ok to signal setup completion
      :ok
