defmodule GraphOS.Core.CodeGraph.ServiceTest do
  ...
  describe "service lifecycle" do
    test "starts and provides status information", %{test_dir: test_dir} do
    ...
    test "can be restarted and rebuilt", %{test_dir: test_dir} do
    ...
    test "can query information about modules", %{test_dir: test_dir} do
      ...
      case result do
        {:ok, info} ->
          ...
          assert length(info.functions) <= 2  # May be less depending on timing
  ...
  describe "subscription" do
    test "can subscribe to events", %{test_dir: test_dir} do
      # Start the service with minimal configuration
      {:ok, _pid} = CodeGraphService.start_link(
        watched_dirs: [],
        file_pattern: "*.ex"
