defmodule GraphOS.Core.MCP.GraphExecutableServerTest do
  ...
  alias MCP.Server.DefaultServer

  # Helper to generate a random session ID
  defp random_session_id, do: "test_session_#{:rand.uniform(1000000)}"

  # Helper to simulate an MCP request
  defp send_request(session_id, method, params) do
    request = %{
      "jsonrpc" => "2.0",
      "id" => "request_#{:rand.uniform(1000000)}",
      "method" => method,
      "params" => params
    }
  ...
  describe "tool listing" do
    setup do
      ...
      {:ok, _} = send_request(session_id, "initialize", %{
        "protocolVersion" => "2024-11-05",
        "clientInfo" => %{
          "id" => "test_client",
          "name" => "Test Client"
  ...
  describe "executable node operations" do
    ...
    test "create and execute an executable node", %{session_id: session_id} do
      ...
      {:ok, create_response} = send_request(session_id, "tools/call", %{
      ...
      # Execute the node
      {:ok, execute_response} = send_request(session_id, "tools/call", %{
        "name" => "graph.execute",
        "arguments" => %{
          "node_id" => "test:node",
          "context" => %{
    ...
    test "query for executable nodes", %{session_id: session_id} do
      ...
      {:ok, _} = send_request(session_id, "tools/call", %{
        "name" => "graph.create_executable",
        "arguments" => %{
          "id" => "query:test",
          "executable_type" => "elixir_code",
      ...
      # Query for nodes
      {:ok, query_response} = send_request(session_id, "tools/call", %{
        "name" => "graph.query",
        "arguments" => %{
          "query" => %{
            "node_properties" => %{
    ...
    test "permission error when executing without permission" do
      ...
      {:ok, _} = send_request(other_session, "initialize", %{
        "protocolVersion" => "2024-11-05",
        "clientInfo" => %{
          "id" => "other_user",
          "name" => "Other User"
      ...
      {:error, error} = send_request(other_session, "tools/call", %{
        "name" => "graph.execute",
        "arguments" => %{
      ...
      assert error.code == -32003  # Permission denied
    end
  end
end