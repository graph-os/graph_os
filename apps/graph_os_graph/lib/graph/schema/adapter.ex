defmodule GraphOS.Graph.Schema.Adapter do
  ...
  def to_protobuf_type(:map), do: :bytes  # Maps are serialized to bytes in protobuf
  def to_protobuf_type(:list), do: :repeated  # Generic repeated field
  def to_protobuf_type({:list, inner_type}), do: {:repeated, to_protobuf_type(inner_type)}
  def to_protobuf_type({:enum, _values}), do: :enum
  def to_protobuf_type(:any), do: :bytes  # Any is serialized to bytes in protobuf
  ...
  def from_protobuf_type(:enum), do: {:enum, []}  # Empty placeholder, actual values should be provided
  ...
  def to_json_schema_type(:any), do: %{}  # Any type in JSONSchema
