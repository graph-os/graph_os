defmodule GraphOS.Store.Algorithm.Weights do
  @moduledoc """
  ...
  def get_edge_weight(edge, _property_name, default_weight) do
    weight =
      if edge.data != nil && Map.has_key?(edge.data, :weight), do: edge.data.weight, else: nil
  ...
  ## Examples
      ...
      iex> GraphOS.Store.Algorithm.Weights.normalize_weights(weights)
  ...
  * `:subtract` - Inverts weights by subtracting from the max value
  ...
  ## Examples
      ...
      iex> GraphOS.Store.Algorithm.Weights.invert_weights(weights, :reciprocal)
      %{"edge1" => 1.0, "edge2" => 0.5, "edge3" => 0.25}

      iex> weights = %{"edge1" => 1, "edge2" => 2, "edge3" => 4}
      iex> GraphOS.Store.Algorithm.Weights.invert_weights(weights, :subtract)
