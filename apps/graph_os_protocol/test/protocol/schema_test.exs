defmodule GraphOS.Protocol.SchemaTest do
  ...
  defmodule TestSchemaAdapter do
    ...
    def upgrade_to_plug(proto_msg, "ListNodes") do
      ...
      query_params =
        if proto_msg.type, do: Map.put(query_params, "type", proto_msg.type), else: query_params

      query_params =
        if proto_msg.limit,
          do: Map.put(query_params, "limit", proto_msg.limit),
          else: query_params

      query_params =
        if proto_msg.cursor,
          do: Map.put(query_params, "cursor", proto_msg.cursor),
          else: query_params
    ...
    def upgrade_to_mcp(proto_msg, "ListNodes") do
      ...
      context =
        if proto_msg.cursor, do: Map.put(context, "cursor", proto_msg.cursor), else: context
