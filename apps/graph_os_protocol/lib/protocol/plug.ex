defmodule GraphOS.Protocol.Plug do
  ...
  def init(opts) do
    ...
    schema_module = Keyword.get(adapter_opts, :schema_module) || 
    ...
    if opts.schema_module do
      ...
      case decode_protobuf(body, [method], opts) do
            ...
            {:ok, response} ->
              ...
              case accept_type do
                ...
                accept when accept in ["application/json", "text/json"] ->
                  # Return as JSON
                  {:ok, json_response} = GraphOS.Protocol.GRPC.upgrade(adapter, response, method, :jsonrpc)
                  conn
                  |> Plug.Conn.put_resp_content_type("application/json")
        ...
        {:error, reason} ->
          ...
          conn
          |> Plug.Conn.put_resp_content_type("application/json")
          |> Plug.Conn.send_resp(400, encode_json(%{error: "Invalid Protocol Buffer: #{inspect(reason)}"}, opts))
      end
    else
      # No schema module available
      conn
      |> Plug.Conn.put_resp_content_type("application/json")
      |> Plug.Conn.send_resp(501, encode_json(%{error: "Protocol Buffer support not configured"}, opts))
    end
  end
  ...
  defp read_body_json(conn, opts) do
    ...
    case decode_json(body, opts) do
      ...
      {:error, _} -> {:ok, %{}, conn} # Default to empty params on parse error
    end
  end
  ...
  defp error_to_response(reason) do
    case reason do
      {:not_found, _} -> {404, %{error: "Not found"}}
      {:unauthorized, _} -> {401, %{error: "Unauthorized"}}
      {:validation_error, details} -> {400, %{error: "Validation error", details: details}}
      {:unknown_path, path} -> {404, %{error: "Unknown path: #{path}"}}
      {:internal_error, _} -> {500, %{error: "Internal server error"}}
      {:decode_error, message} -> {400, %{error: "Protocol Buffer decode error", details: message}}
      {:unknown_message_type, rpc_name} -> {400, %{error: "Unknown message type for RPC: #{rpc_name}"}}
      :schema_module_missing -> {501, %{error: "Protocol Buffer support not configured"}}
      _ -> {500, %{error: "Unexpected error"}}
    end
  end
end